...
const EvaluateRule = () => {
  ...
  useEffect(() => {
    const getRuleIds = async () => {
      try {
        const response = await fetch('http://localhost:8000/api/rules/all');
...
<div className="flex flex-col space-y-6 w-[30vw] bg-[#FDF6E3] rounded-lg shadow-md p-6">
      ...
      <form onSubmit={handleEvaluateRule} className="flex flex-col space-y-4">
        ...
        <button
          type="submit"
          className="bg-[#FF6B6B] text-white p-3 rounded-lg shadow-md  focus:outline-none focus:ring-2 focus:ring-blue-500 transition-transform transform hover:scale-105"
        >
          Evaluate Rule
        </button>
      </form>
